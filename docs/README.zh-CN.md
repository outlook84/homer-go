...
本项目为追求「Homer 般的使用体验，但希望以单一 Go 服务运行」的用户设计，并为支持的智能卡片提供服务端状态收集功能。

## 与 Homer 的主要区别

homer-go 兼容大部分 Homer 的 YAML 配置格式，因此现有的 Homer 用户通常可以直接使用现有的 `config.yml`。其主要区别在于：

- **运行方式**：homer-go 是一个 Go 编写的 HTTP 服务器；而 Homer 是一个需要 Web 服务器托管的静态 Vue 应用。
...
- 基于 YAML 的仪表盘配置
- 支持分组、服务卡片、标签、图标、Logo、快速链接及自定义卡片颜色
- 内置浅色、深色及跟随系统的自动主题切换
- 提供列布局和列表布局偏好设置
